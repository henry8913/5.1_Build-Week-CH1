import React from 'react';
import { Container, Row, Col, Button } from 'react-bootstrap';
import './css/ActivityComponent.css';

export default function ActivityComponent() {
  return (
    <Container className="activity-container">
      <div className="activity-header">
        <div className="activity-title">
          <h2>Attività</h2>
          <span>4.915 follower</span>
        </div>
        <Button variant="outline-secondary">+ Segui</Button>
      </div>

      <div className="activity-tabs">
        <Button variant="success" className="active">Post</Button>
        <Button variant="light">Commenti</Button>
        <Button variant="light">Immagini</Button>
      </div>

      <div className="activity-posts">
        <Row>
          <Col md={6}>
            <div className="post-item">
              <div className="post-header">
                <img src="data:image/jpeg;base64," className="profile-pic" alt="Profile" />
                <div className="post-meta">
                  <h6>Ivan Ranza · 3° e oltre</h6>
                  <p>Co-Founder & CEO at EPICODE & EPIC...</p>
                  <span>3 giorni · <i className="fas fa-globe"></i></span>
                </div>
                <Button variant="light" className="more-options">...</Button>
              </div>
              <div className="post-content">
                <p>Un ottimo editoriale di Maurizio Belpietro, in poche righe fa un'analisi lucida della scuola italiana. Ogni riferimento a persone o fatti è puramente...</p>
                <img src="https://media.licdn.com/dms/image/v2/D4D22AQFFwQ9vxu-GWQ/feedshare-shrink_800/B4DZVhGyFDHwAg-/0/1741090924273?e=1747267200&v=beta&t=OqYrKsn0thpALVsw9m5k_Np7Y7Gqq7lpisevWlTowr0" className="post-image" alt="Post content" />
              </div>
              <div className="post-actions">
                <Button variant="light"><i className="far fa-thumbs-up"></i></Button>
                <Button variant="light"><i className="far fa-comment"></i></Button>
                <Button variant="light"><i className="fas fa-share"></i></Button>
                <Button variant="light"><i className="far fa-paper-plane"></i></Button>
              </div>
              <div className="post-reactions">
                <span>15</span>
              </div>
            </div>
          </Col>
          <Col md={6}>
            <div className="post-item">
              <div className="post-header">
                <img src="data:image/jpeg;base64," className="profile-pic" alt="Profile" />
                <div className="post-meta">
                  <h6>Ivan Ranza · 3° e oltre</h6>
                  <p>Co-Founder & CEO at EPICODE & EPIC...</p>
                  <span>1m · <i className="fas fa-globe"></i></span>
                </div>
                <Button variant="light" className="more-options">...</Button>
              </div>
              <div className="post-content">
                <p>Quando qualche fenomeno vi dice che 'l'Italia potrebbe vivere di turismo' e poi vi chiedete perché la produttività italiana è bassa, guardate...</p>
                <img src="https://media.licdn.com/dms/image/v2/D4D22AQGuGlV7749kxg/feedshare-shrink_800/B4DZVw3q9VHwAg-/0/1741355398368?e=1747267200&v=beta&t=atILhbvb3YHbCQw_JSajUg2AEba0CjftBsrCp_rIQzM" className="post-image" alt="Post content" />
              </div>
              <div className="post-actions">
                <Button variant="light"><i className="far fa-thumbs-up"></i></Button>
                <Button variant="light"><i className="far fa-comment"></i></Button>
                <Button variant="light"><i className="fas fa-share"></i></Button>
                <Button variant="light"><i className="far fa-paper-plane"></i></Button>
              </div>
              <div className="post-reactions">
                <span>43 · 1 commento · 1 diffusione post</span>
              </div>
            </div>
          </Col>
        </Row>
      </div>
      <div className="show-all-posts">
        <a href="#" className="d-block text-center text-decoration-none">Mostra tutti i post →</a>
      </div>
    </Container>
  );
}
